...
export default function CapturaInventario() {
  ...
  const aplicarVistaDiferenciasBrigada =
    esBrigada && (Number(estatus) === 3 || Number(estatus) === 7);
  ...
  const getFirmaParametros = (p = null) => {
    const { cia, almacen, fecha, empleado, estatus } =
      p || getParametrosEfectivos();
  ...
  const iniciarCaptura = async () => {
    ...
    try {
      ...
      const conteoAsignadoActual = Number(nroAsignado || nroConteo || 1);

      const procesoFinalizado =
        estatusBackend >= 4 && conteoAsignadoActual !== 7;
      ...
      const estatusReal = conteoAsignadoActual;
      ...
      if (r1.data.ir_comparar) {
        ...
        navigate("/comparar", {
          ...
          state: {
            ...
            empleado: emp,
            cia,
            estatus: r1.data.nro_conteo_asignado || estatusReal,
          },
        });
  ...
  const cambiarCantidad = (uid, valor) => {
    const nuevo = [...datos];
    const idx = nuevo.findIndex((x) => `${x.ItemCode}-${x.almacen}` === uid);
  ...
  const confirmarInventario = async () => {
    ...
    try {
      ...
      for (let i = 0; i < lotes.length; i++) {
        ...
        payload.append(
          "estatus",
          asignacionCargada ? parseInt(nroConteo) : parseInt(estatusFinal),
        );
      ...
      navigate("/comparar", {
        state: (() => {
          ...
          return {
            ...
            empleado: p.empleado,
            cia: p.cia,
            estatus: asignacionCargada
              ? parseInt(nroConteo)
              : parseInt(estatusFinal),
          };
        })(),
      });
    } catch (error) {
    ...
    return total;
        ...
        if (cantidad !== undefined) {
          ...
          const elemento = document.getElementById(
            `fila-${producto.ItemCode}-${producto.almacen}`,
          );
          if (elemento) {
  ...
  return (
    <div className="w-full max-w-none mx-auto px-3 md:px-6">
      ...
      <div className="bg-white border border-gray-300 rounded-lg shadow-sm p-4 mb-6">
        {asignacionCargada ? (
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-700">
              <strong>Conteo asignado:</strong>{" "}
              {Number(nroConteo) === 7 ? "Validación Físico vs SAP" : nroConteo}
            </p>
      ...
      {modo && (
        <div className="mt-8">
          <p
            className={`flex items-center gap-2 mb-4 text-lg font-medium ${bloqueado ? "text-red-600" : "text-green-600"}`}
          >
            {mensajeModo}
          </p>
          ...
          {(!esDispositivoMovilOTablet || mostrarModoRapido) && (
            <div className="relative bg-gradient-to-br from-white via-gray-50 to-white border border-gray-200 rounded-3xl shadow-2xl p-10 mb-10">
                  ...
                  <ul className="space-y-3 text-sm">
                    {historial.map((h, i) => (
                      ...
                      >
                        ...
                        <span className="flex-1 text-gray-700 ml-4 font-medium truncate">
                          {h.nombre}
                        </span>
              ...
              <p className="text-center mt-8 text-sm text-gray-600 italic">
                Usa el lector o escribe manualmente y confirma con{" "}
          ...
          {loadingInventario ? (
            ...
            <div
              className="w-full overflow-x-auto overflow-y-auto max-h-[70vh] border rounded-lg shadow-md bg-white"
