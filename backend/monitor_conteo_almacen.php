...
FROM SAP_PROCESOS.dbo.CAP_INVENTARIO
WHERE cias = '$cia'
  AND almacen = '$almacen'
...
function contarConteo($conn, $cia, $almacen, $fecha, $conteo) {
    ...
    FROM SAP_PROCESOS.dbo.CAP_INVENTARIO_CONTEOS c
    INNER JOIN SAP_PROCESOS.dbo.CAP_INVENTARIO i
...
INNER JOIN SAP_PROCESOS.dbo.CAP_INVENTARIO i
    ON i.id = c.id_inventario
LEFT JOIN SAP_PROCESOS.dbo.usuarios u
...
FROM (
    ...
    FROM SAP_PROCESOS.dbo.CAP_INVENTARIO i
    LEFT JOIN SAP_PROCESOS.dbo.CAP_INVENTARIO_CONTEOS c
...
FROM SAP_PROCESOS.dbo.CAP_INVENTARIO_SESIONES s
WHERE s.cia = '$cia'
...
SELECT TOP 1
    *
FROM SAP_PROCESOS.dbo.CAP_INVENTARIO_CIERRE
WHERE cia = '$cia'
  AND almacen = '$almacen'
...
if ($idCierre > 0) {
    ...
    SELECT TOP 1
        *
    FROM SAP_PROCESOS.dbo.CAP_SAP_SIGNAL
